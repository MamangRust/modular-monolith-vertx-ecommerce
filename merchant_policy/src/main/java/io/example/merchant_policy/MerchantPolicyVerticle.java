package io.example.merchant_policy;
...
import io.example.merchant_policy.handler.MerchantPolicyCommandHandler;
import io.example.merchant_policy.handler.MerchantPolicyQueryHandler;
...
import io.example.merchant_policy.repository.impl.MerchantPoliciesCommandRepositoryImpl;
import io.example.merchant_policy.repository.impl.MerchantPoliciesQueryRepositoryImpl;
import io.example.merchant_policy.repository.impl.MerchantQueryRepositoryImpl;
...
public class MerchantPolicyVerticle extends AbstractVerticle {
  private static final Logger log = LoggerFactory.getLogger(MerchantPolicyVerticle.class);
  ...
  public static void main(String[] args) {
    ...
    vertx.deployVerticle(new MerchantPolicyVerticle(), options)
        .onSuccess(id -> {
          log.info("✅ MerchantPolicy Service successfully deployed! ID: {}", id);
          log.info("🚀 gRPC Server running on port 50068");
        })
        .onFailure(err -> {
          log.error("❌ Failed to deploy MerchantPolicyVerticle", err);
        });
  }
  ...
  public void start(Promise<Void> startPromise) {
    ...
    if (!telConfig.containsKey("service.name")) {
      telConfig.put("service.name", "merchant-policy-service");
    }
    telemetryConfig = new TelemetryConfig(telConfig);
    OpenTelemetry openTelemetry = telemetryConfig.initialize();
    TracingMetrics tracingMetrics = new TracingMetrics(openTelemetry, "merchant-policy-service");
    ...
    var cmdHandler = new MerchantPolicyCommandHandler(cmdService);

    int port = cfg.getGrpcPort() > 0 ? cfg.getGrpcPort() : 50068;

    startGrpcServer(queryHandler, cmdHandler, port)
        .onSuccess(v -> {
          log.info("MerchantPolicyVerticle fully initialized with CQRS. Listening for gRPC on port {}", port);
  ...
  public void stop(Promise<Void> stopPromise) {
    ...
    log.info("MerchantPolicyVerticle successfully stopped!");
  ...
  private Future<Void> startGrpcServer(
      MerchantPolicyQueryHandler queryHandler,
      MerchantPolicyCommandHandler cmdHandler,
      int grpcPort) {
