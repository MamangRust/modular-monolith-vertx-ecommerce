...
public class OrderStatsRepositoryImpl implements OrderStatsRepository {
    ...
    private Tuple getMonthlyDateRange(int year, int month) {
        ...
        LocalDateTime startPrev = startPrevDate.atStartOfDay();
        LocalDateTime endPrev = endPrevDate.atTime(LocalTime.MAX);
                ...
                .preparedQuery(
                        """
                                WITH monthly_revenue AS (
                                    ...
                                    SELECT EXTRACT(YEAR FROM $1::date)::TEXT AS year, EXTRACT(MONTH FROM $1::date)::integer AS month, TO_CHAR($1::date, 'FMMonth') AS month_name
                                    UNION
                                    SELECT EXTRACT(YEAR FROM $3::date)::TEXT AS year, EXTRACT(MONTH FROM $3::date)::integer AS month, TO_CHAR($3::date, 'FMMonth') AS month_name
                                )
                                SELECT COALESCE(am.year, EXTRACT(YEAR FROM $1::date)::TEXT) AS year,
                                       COALESCE(am.month_name, TO_CHAR($1::date, 'FMMonth')) AS month,
                                       COALESCE(mr.total_revenue, 0::BIGINT)::BIGINT AS total_revenue
                ...
                .preparedQuery(
                        """
                                WITH yearly_revenue AS (
                                    SELECT EXTRACT(YEAR FROM o.created_at)::integer AS year, COALESCE(SUM(o.total_price), 0)::BIGINT AS total_revenue
                                    FROM orders o JOIN order_items oi ON o.order_id = oi.order_id
                                    WHERE o.deleted_at IS NULL AND oi.deleted_at IS NULL
                ...
                .preparedQuery(
                        """
                                WITH monthly_revenue AS (
                                    ...
                                    SELECT EXTRACT(YEAR FROM $1::date)::TEXT AS year, EXTRACT(MONTH FROM $1::date)::integer AS month, TO_CHAR($1::date, 'FMMonth') AS month_name
                                    UNION SELECT EXTRACT(YEAR FROM $3::date)::TEXT AS year, EXTRACT(MONTH FROM $3::date)::integer AS month, TO_CHAR($3::date, 'FMMonth') AS month_name
                                )
                                SELECT COALESCE(am.year, EXTRACT(YEAR FROM $1::date)::TEXT) AS year,
                                       COALESCE(am.month_name, TO_CHAR($1::date, 'FMMonth')) AS month,
                                       COALESCE(mr.total_revenue, 0::BIGINT)::BIGINT AS total_revenue
    ...
    @Override
    public Future<List<OrderMonthlyTotalRevenue>> getMonthlyTotalRevenueByMerchant(Integer merchantId, int year,
            int month) {
    ...
    private List<OrderMonthlyTotalRevenue> mapMonthlyTotalRevenue(RowSet<Row> rows) {
        ...
        for (Row r : rows) {
            list.add(new OrderMonthlyTotalRevenue(
                    r.getString("year"),
                    r.getString("month"),
                    asLong(r.getValue("total_revenue"))));
        }
        return list;
    }
