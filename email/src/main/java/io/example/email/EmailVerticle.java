...
public class EmailVerticle extends AbstractVerticle {
  ...
  public static void main(String[] args) {
    ...
    io.vertx.core.DeploymentOptions options = new io.vertx.core.DeploymentOptions()
        .setConfig(new JsonObject().put("service.name", "email-service"));

    vertx.deployVerticle(new EmailVerticle(), options)
        .onSuccess(id -> log.info("✅ Email Service successfully deployed! ID: {}", id))
        .onFailure(err -> log.error("❌ Failed to deploy EmailVerticle", err));
  }
  ...
  public void start(Promise<Void> startPromise) {
    ...
    retryProcessor = new EmailRetryProcessor(vertx, retryConsumer, mailClient, dedupGuard, retryPublisher,
        System.getenv().getOrDefault("SMTP_FROM", "no-reply@vertx-ecommerce.com"), emailMetrics, emailTracing);
  ...
  private Future<Void> sendEmail(JsonObject payload, Context otelContext) {
    MailMessage message = new MailMessage()
        .setFrom(System.getenv().getOrDefault("SMTP_FROM", "no-reply@vertx-ecommerce.com"))
