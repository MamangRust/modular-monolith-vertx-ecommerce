...
public class UserRepositoryImpl implements UserRepository {
    ...
    @Override
    public Future<AuthUser> updateUserIsVerified(UpdateUserVerifiedRequest request) {
        return pool.preparedQuery("""
                UPDATE users SET is_verified = $1, updated_at = CURRENT_TIMESTAMP
                WHERE user_id = $2 AND deleted_at IS NULL
                RETURNING *
                """)
                .execute(Tuple.of(request.isVerified(), request.getUserId()))
                .map(rows -> rows.iterator().hasNext() ? AuthUser.fromRow(rows.iterator().next()) : null);
    }

    @Override
    public Future<AuthUser> updateUserPassword(UpdateUserPasswordRequest request) {
        return pool.preparedQuery("""
