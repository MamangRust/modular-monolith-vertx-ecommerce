...
class UserRepositoryImplTest {
  ...
  private void stubSingleRow() {
    // Finite iterator: exactly one row, then exhausted. Using an infinite
    // `hasNext() -> true` stub here caused AuthUser.fromRowsWithRoles (which
    // iterates the RowSet) to loop forever until OutOfMemoryError.
  ...
  void updateUserIsVerifiedSuccess(VertxTestContext ctx) {
    ...
    UpdateUserVerifiedRequest req = UpdateUserVerifiedRequest.builder().userId(1).isVerified(true).build();

    repo.updateUserIsVerified(req)
        .onComplete(ctx.succeeding(user -> ctx.verify(() -> {
          assertThat(user).isNotNull();
          assertThat(user.getUserId()).isEqualTo(1);
          ctx.completeNow();
  ...
  void updateUserIsVerifiedNotFound(VertxTestContext ctx) {
    ...
    when(preparedQuery.execute(any(Tuple.class))).thenReturn(Future.succeededFuture(rowSet));
        ...
        UpdateUserVerifiedRequest req = UpdateUserVerifiedRequest.builder().userId(99).isVerified(true).build();
  ...
  void updateUserPasswordSuccess(VertxTestContext ctx) {
    ...
    UpdateUserPasswordRequest req= UpdateUserPasswordRequest.builder().userId(1).password("new-hashed-password").build();

    repo.updateUserPassword(req)
        .onComplete(ctx.succeeding(user -> ctx.verify(() -> {
          assertThat(user).isNotNull();
          assertThat(user.getUserId()).isEqualTo(1);
          ctx.completeNow();
