...
class PasswordResetServiceTest {
  ...
  void resetPasswordWithCachedToken(VertxTestContext ctx) {
    ...
    UpdateUserPasswordRequest req = UpdateUserPasswordRequest.builder().userId(1).password("new-password").build();

    when(redisService.get("resetToken:valid-token")).thenReturn(Future.succeededFuture("1"));
    when(userRepository.updateUserPassword(req))
  ...
  void verifyCodeSuccess(VertxTestContext ctx) {
    ...
    when(userRepository.updateUserIsVerified(req)).thenReturn(Future.succeededFuture(user));
