...
class IdentityServiceTest {
  ...
  void getMeCacheHit(VertxTestContext ctx) {
    ...
    when(redisService.getJson("auth:user:1", AuthUser.class)).thenReturn(Future.succeededFuture(cachedUser));
  ...
  void getMeCacheMiss(VertxTestContext ctx) {
    ...
    when(redisService.getJson("auth:user:1", AuthUser.class)).thenReturn(Future.succeededFuture(null));
    ...
    service.getMe(1)
        .onComplete(ctx.succeeding(user -> ctx.verify(() -> {
          assertThat(user).isNotNull();
          assertThat(user.getUserId()).isEqualTo(1);
          verify(redisService).setJson("auth:user:1", dbUser, Duration.ofMinutes(5));
          ctx.completeNow();
        })));
  }
  ...
  void getMeUserNotFound(VertxTestContext ctx) {
    ...
    when(redisService.getJson("auth:user:99", AuthUser.class)).thenReturn(Future.succeededFuture(null));
