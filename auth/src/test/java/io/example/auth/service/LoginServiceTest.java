...
class LoginServiceTest {
  ...
  void loginSuccess(VertxTestContext ctx) {
    ...
    // LoginService memanggil overload 2-argumen createAccessToken(userId, roles);
    // aVerifiedUser() tidak set roles → AuthUser default roles = empty list.
    when(tokenService.createAccessToken(1, user.getRoles())).thenReturn("access-token");
