...
psql_gql() {
  "${COMPOSE[@]}" exec -T postgres psql -X -v ON_ERROR_STOP=1 -U DRAGON -d ECOMMERCE "$@"
}
...
# Register through the GraphQL endpoint (the same one production clients use).
...
# Pull the OTP directly from postgres (the same shortcut run-e2e.sh takes).
psql_gql -At -c \
  "SELECT verification_code FROM users WHERE email = '$(sql_literal "$E2E_EMAIL")';" \
  >/tmp/vertx-ecommerce-gql-e2e-code
...
# Pull access token for authenticated tests
LOGIN_PAYLOAD=$(cat <<JSON
{"query":"mutation Login(\$e:String!,\$p:String!){ login(email:\$e,password:\$p){ status accessToken refreshToken } }","variables":{"e":"$E2E_EMAIL","p":"$E2E_PASSWORD"}}
JSON
)
LOGIN_RESPONSE=$(curl --silent --show-error --max-time 10 \
  -H 'Content-Type: application/json' \
  -d "$LOGIN_PAYLOAD" \
  "$GRAPHQL_URL")
ACCESS_TOKEN=$(echo "$LOGIN_RESPONSE" | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['login']['accessToken'])" 2>/dev/null || echo "")
RUN_ID="e2e-$(date +%s)"
