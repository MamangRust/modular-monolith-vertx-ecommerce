...
public class SliderCommandRepositoryImpl implements SliderCommandRepository {
    ...
    public SliderCommandRepositoryImpl(Pool client) {
        this.client = client;
    }
    ...
    @Override
    public Future<Void> deletePermanent(Long sliderId) {
        return client
    ...
    public Future<Integer> restoreAll() {
        return client
                .preparedQuery("UPDATE sliders SET deleted_at = NULL, updated_at = CURRENT_TIMESTAMP WHERE deleted_at IS NOT NULL")
                .execute()
                .map(RowSet::rowCount);
    }
