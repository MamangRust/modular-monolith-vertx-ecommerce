...
import io.example.banner.handler.BannerCommandHandler;
import io.example.banner.handler.BannerQueryHandler;
import io.example.banner.repository.BannerCommandRepository;
import io.example.banner.repository.BannerQueryRepository;
import io.example.banner.repository.impl.BannerCommandRepositoryImpl;
import io.example.banner.repository.impl.BannerQueryRepositoryImpl;
...
public class BannerVerticle extends AbstractVerticle {
  ...
  public static void main(String[] args) {
    ...
    vertx.deployVerticle(new BannerVerticle(), options)
        .onSuccess(id -> {
          log.info("✅ Banner Service successfully deployed! ID: {}", id);
          log.info("🚀 gRPC Server running on port 8082");
        })
        .onFailure(err -> {
          log.error("❌ Failed to deploy BannerVerticle", err);
        });
  }
  ...
  public void start(Promise<Void> startPromise) {
    ...
    if (!telConfig.containsKey("service.name")) {
        telConfig.put("service.name", "banner-service");
    }
    telemetryConfig = new TelemetryConfig(telConfig);
    OpenTelemetry openTelemetry = telemetryConfig.initialize();
    TracingMetrics tracingMetrics = new TracingMetrics(openTelemetry, "banner-service");
    ...
    BannerQueryService queryService = new BannerQueryServiceImpl(queryRepo, redisService, tracingMetrics);
    BannerCommandService cmdService = new BannerCommandServiceImpl(cmdRepo, redisService, tracingMetrics);

    // 5. Initialize Handlers
    var queryHandler = new BannerQueryHandler(queryService);
    var cmdHandler = new BannerCommandHandler(cmdService);
    
    int port = cfg.getGrpcPort();

    startGrpcServer(queryHandler, cmdHandler, port)
        .onSuccess(v -> {
          log.info("BannerVerticle fully initialized with CQRS. Listening for gRPC on port {}", port);
  ...
  private Future<Void> startGrpcServer(BannerQueryHandler queryHandler, BannerCommandHandler cmdHandler, int grpcPort) {
