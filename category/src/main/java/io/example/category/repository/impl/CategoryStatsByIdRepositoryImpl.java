...
public class CategoryStatsByIdRepositoryImpl implements CategoryStatsByIdRepository {
    ...
    @Override
    public Future<List<CategoriesMonthlyTotalPrice>> getMonthlyTotalPriceById(CategoryCommon.FindYearMonthTotalPriceById req) {
        Tuple args = getMonthlyTuple(req.getYear(), req.getMonth()).addLong((long) req.getCategoryId());
        return client
                .preparedQuery("""
    ...
    @Override
    public Future<List<CategoriesYearlyTotalPrice>> getYearlyTotalPriceById(CategoryCommon.FindYearTotalPriceById req) {
        return client
                .preparedQuery("""
                        WITH yearly_data AS (
                        ...
                        """)
                .execute(Tuple.of(req.getYear(), (long) req.getCategoryId()))
                .map(this::mapYearlyTotalPrice);
    }

    @Override
    public Future<List<CategoriesMonthPrice>> getMonthlyCategoryById(CategoryCommon.FindYearCategoryById req) {
        ...
        return client
                .preparedQuery("""
                        ...
                        """)
                .execute(Tuple.of(refTs, (long) req.getCategoryId()))
                .map(this::mapMonthPrice);
    }

    @Override
    public Future<List<CategoriesYearPrice>> getYearlyCategoryById(CategoryCommon.FindYearCategoryById req) {
        Timestamp refTs = Timestamp.valueOf(LocalDateTime.of(req.getYear(), 1, 1, 0, 0));
        return client
                .preparedQuery("""
    ...
    private Tuple getMonthlyTuple(int year, int month) {
        int actualMonth = month > 0 ? month : 1;
        LocalDate startDate = LocalDate.of(year, actualMonth, 1);
    ...
    private List<CategoriesMonthlyTotalPrice> mapMonthlyTotalPrice(RowSet<Row> rows) {
        List<CategoriesMonthlyTotalPrice> list = new ArrayList<>();
        for (Row r : rows) {
            list.add(new CategoriesMonthlyTotalPrice(
                    r.getString("year"),
                    r.getString("month"),
    ...
    private List<CategoriesYearlyTotalPrice> mapYearlyTotalPrice(RowSet<Row> rows) {
        List<CategoriesYearlyTotalPrice> list = new ArrayList<>();
        for (Row r : rows) {
            list.add(new CategoriesYearlyTotalPrice(
                    r.getString("year"),
                    r.getLong("total_revenue")));
        }
        return list;
    ...
    private List<CategoriesMonthPrice> mapMonthPrice(RowSet<Row> rows) {
        List<CategoriesMonthPrice> list = new ArrayList<>();
        for (Row r : rows) {
            list.add(new CategoriesMonthPrice(
                    r.getString("month"),
    ...
    private List<CategoriesYearPrice> mapYearPrice(RowSet<Row> rows) {
        List<CategoriesYearPrice> list = new ArrayList<>();
        for (Row r : rows) {
            list.add(new CategoriesYearPrice(
                    r.getString("year"),
                    r.getInteger("category_id"),
                    r.getString("category_name"),
                    r.getInteger("order_count"),
