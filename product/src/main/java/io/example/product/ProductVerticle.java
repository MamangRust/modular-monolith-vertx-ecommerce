...
public class ProductVerticle extends AbstractVerticle {
    ...
    public void start(Promise<Void> startPromise) {
        ...
        // 2. Initialize Repositories and client-side gRPC dependencies
        ...
        var categoryQueryClient = new pb.category.VertxCategoryQueryServiceGrpcClient(grpcClient, addrCategory);
        ...
        final int finalPort = cfg.getGrpcPort() == 8083 ? 50058 : cfg.getGrpcPort();

        startGrpcServer(queryHandler, cmdHandler, finalPort)
                .onSuccess(v -> {
                    log.info("ProductVerticle fully initialized with CQRS. Listening for gRPC on port {}", finalPort);
                    startPromise.complete();
                })
                .onFailure(err -> {
                    log.error("Failed to bind gRPC server on port {}", finalPort, err);
